...
export class HuiEditCard extends LitElement {
  ...
  private get _cardJSON(): LovelaceCardConfig | undefined {
    return this._cardConfig;
  }

  private set _cardJSON(config: LovelaceCardConfig | undefined) {
    this._cardConfig = config;
  }

  private get _cardYAML(): string | undefined {
    return this.cardConfig ? yaml.safeDump(this._cardJSON) : undefined;
  }

  private set _cardYAML(yml: string | undefined) {
    this._cardJSON = yaml.safeLoad(yml);
  }
  ...
  private async _save(): Promise<void> {
    ...
    try {
      await this.saveCard!(this._cardJSON!);
      this.closeDialog!();
    } catch (err) {
  ...
  private _handleYamlChanged(ev: CustomEvent): void {
    this._cardYAML = ev.detail.value;
    try {
      this._updatePreview(this._cardJSON!);
      this._configState = "OK";
    } catch (err) {
  ...
  private async _toggleEditor(): Promise<void> {
    if (this._uiEditor) {
      ...
      this._uiEditor = true;
      if (this._cardJSON!.type !== this._cardType) {
        const success = await this._loadConfigElement(this._cardJSON!);
        if (!success) {
          this._loadedDialog();
        }
        this._cardType = this._cardJSON!.type;
      } else {
        this._configElement.setConfig(this._cardJSON!);
      }
    }
    this._resizeDialog();
  }
  ...
  private _isConfigChanged(): boolean {
    ...
    return JSON.stringify(this._cardJSON) !== JSON.stringify(this.cardConfig);
  }
